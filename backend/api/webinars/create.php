...
$meetingId   = isset($data['meeting_id'])   ? trim($data['meeting_id'])   : null;
$isLive      = isset($data['is_live'])      ? (int)$data['is_live']       : 0;
...
try {
    ...
    $stmt = $db->prepare("
        INSERT INTO webinars (title, mentor_name, date_time, stream_link, meeting_id, is_live, recording_url) 
        VALUES (?, ?, ?, ?, ?, ?, ?)
    ");
    $stmt->execute([$title, $mentorName, $dateTime, $streamLink, $meetingId, $isLive, $recordingUrl]);
    ...
    if ($webinar) {
        ...
        $webinar['computed_status'] = 'Upcoming';
        $webinar['meeting_id'] = $webinar['meeting_id'] ?? null;
    }
