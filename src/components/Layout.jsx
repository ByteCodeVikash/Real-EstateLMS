...
import { 
  ...
  LogOut, Bell, Award, TrendingUp, Search, Flame, ChevronDown, User, ShieldAlert 
} from 'lucide-react';
import { cn } from './UI';
...
const menuItems = [
  ...
  { icon: Award, label: 'Certificates', path: '/courses' }, // Map to courses where certification block is active
  { icon: TrendingUp, label: 'Analytics', path: '/dashboard' }, // Map to dashboard analytics graph
  ...
  return (
    <aside className="fixed left-0 top-0 h-screen w-64 bg-premium-dark border-r border-slate-900 hidden lg:flex flex-col z-50 shadow-xl text-slate-400">
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              cn(
                ...
                isActive && item.label !== 'Analytics' && item.label !== 'Settings' && item.label !== 'Certificates'
      ...
      <div className="p-4 border-t border-slate-900">
        <Link to="/" className="flex items-center gap-3 px-4 py-3 w-full rounded-xl text-slate-500 hover:bg-red-500/10 hover:text-red-400 transition-all duration-300 font-bold text-xs uppercase tracking-wider">
...
export const Navbar = () => {
  ...
  return (
    <header className="h-20 bg-white/95 backdrop-blur-md border-b border-premium-border fixed top-0 right-0 left-0 lg:left-64 z-40 px-8 flex items-center justify-between shadow-sm">
      {/* Search Bar */}
      ...
      <div className="flex items-center gap-6">
        ...
        <div className="bg-amber-50 border border-amber-100 px-3.5 py-1.5 rounded-xl flex items-center gap-1.5 shadow-sm shrink-0">
          <Flame className="w-4 h-4 text-amber-500 fill-current animate-pulse" />
          <span className="text-[10px] text-amber-700 font-black uppercase tracking-wider">7 Day Streak</span>
        </div>

        {/* Notifications Icon */}
        <Link to="/notifications" className="relative p-2 text-slate-400 hover:text-premium-heading transition-colors group">
        ...
        <div className="relative">
          <button 
            ...
            className="flex items-center gap-3 pl-6 border-l border-premium-border cursor-pointer group focus:outline-none"
          >
            ...
            <div className="w-10 h-10 rounded-full border-2 border-premium-border group-hover:border-premium-accent transition-all overflow-hidden shadow-sm shrink-0">
              <img
