...
import { Video, Clock, Users, ArrowRight, Play, User, Star } from 'lucide-react';
    ...
    <div className="space-y-8 animate-in text-left">
      ...
      <GlassCard className="p-0 overflow-hidden relative group border border-slate-900 shadow-2xl">
        ...
        <div className="relative z-10 p-8 md:p-12 max-w-2xl space-y-6">
          <Badge variant="danger" className="animate-pulse px-4 py-1.5 text-xs font-black rounded-lg tracking-widest bg-red-500/10 text-red-400 border border-red-500/20">
            <span className="flex items-center gap-2">
              <span className="w-2 h-2 bg-red-500 rounded-full"></span> LIVE BROADCAST ACTIVE
            </span>
          </Badge>
          <h2 className="text-3xl md:text-4xl font-black text-white leading-tight">
            Live Underwriting Audit: NYC &amp; Dallas Multi-Family Analysis
          </h2>
          ...
          <div className="flex flex-wrap items-center gap-8 pt-4 border-t border-slate-800">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-bold text-white leading-none">1,240+</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          ...
          {[
            { day: "19", month: "May", title: "Luxury Mandates: Pitching to High-Net-Worth Sellers", host: "Elena Rodriguez", len: "90 Minutes" },
            { day: "22", month: "May", title: "Off-Market Distressed Sourcing & ARV Calculations", host: "Marcus Thorne", len: "120 Minutes" },
            { day: "26", month: "May", title: "GP/LP waterfall Splitting & Complex Syndication", host: "Robert Sterling", len: "90 Minutes" }
          ].map((item, i) => (
            <GlassCard key={i} className="group hover:bg-slate-50/80 bg-white border border-premium-border p-6 shadow-sm duration-300">
              <div className="flex flex-col md:flex-row items-center gap-8">
                <div className="shrink-0 text-center md:border-r md:border-slate-100 md:pr-8">
                  <p className="text-3xl font-black text-premium-accent">{item.day}</p>
                  <p className="text-xs font-black uppercase text-slate-400 tracking-widest mt-0.5">{item.month}</p>
                  <p className="text-[10px] text-slate-400 mt-2 font-mono font-bold">11:30 AM EST</p>
                </div>
                <div className="flex-1 text-center md:text-left space-y-2">
                  ...
                  <div className="flex flex-wrap items-center justify-center md:justify-start gap-4">
                    ...
                    <span className="flex items-center gap-1.5 text-xs text-slate-400 font-bold">
                      <Clock className="w-4 h-4 text-premium-accent" /> {item.len}
                    </span>
